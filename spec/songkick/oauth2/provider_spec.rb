  ...
  describe "access grant request" do
    ...
    describe "when there is already a pending authorization from the user" do
      ...
      it "immediately redirects with the code" do
        ...
        expect(response['location']).to eql('https://client.example.com/cb?code=pending_code')
      end
      ...
      describe "when the client is requesting scopes it already has access to" do
        ...
        it "immediately redirects with the code" do
          response = get(params)
          expect(response.code.to_i).to eql(302)
          expect(response['location']).to eql('https://client.example.com/cb?code=pending_code&scope=offline_access')
        end
      end
      ...
      describe "and the authorization does not have a code" do
        ...
        it "generates a new code and redirects" do
          ...
          response = get(params)
          expect(response.code.to_i).to eql(302)
          expect(response['location']).to eql('https://client.example.com/cb?code=new_code')
        end
      end
    ...
    describe "when there is already a completed authorization from the user" do
      ...
      describe "for token requests" do
        ...
        it "immediately redirects with a new token" do
          ...
          response = get(params)
          expect(response.code.to_i).to eql(302)
          expect(response['location']).to eql('https://client.example.com/cb#access_token=new_access_token')
        end
        ...
        describe "with an invalid client_id" do
          ...
          it "does not generate any new tokens" do
            expect(Songkick::OAuth2).not_to receive(:random_string)
            get(params)
          end
        end
      end
      ...
      it "does not create a new Authorization" do
        get(params)
        expect(Songkick::OAuth2::Model::Authorization.count).to eql(1)
      end
    ...
    describe "with a client_id and a bad redirect_uri" do
      ...
      it "redirects to the client's registered redirect_uri" do
        response = get(params)
        expect(response.code.to_i).to eql(302)
        expect(response['location']).to eql('https://client.example.com/cb?error=invalid_request&error_description=Missing+required+parameter+response_type')
      end
    end
    ...
    describe "with an invalid request" do
      ...
      describe "with a state parameter" do
        ...
        it "redirects to the client, including the state param" do
          response = get(params)
          expect(response.code.to_i).to eql(302)
          expect(response['location']).to eql("https://client.example.com/cb?error=invalid_request&error_description=Missing+required+parameter+response_type&state=Facebook%0Amesses+this%0Aup")
        end
      end
    end
  end
  ...
  describe "authorization confirmation from the user" do
    ...
    describe "without the user's permission" do
      ...
      it "redirects to the client with an error" do
        ...
        expect(response.code.to_i).to eql(302)
        expect(response['location']).to eql('https://client.example.com/cb?error=access_denied&error_description=The+user+denied+you+access')
      end
    end
    ...
    describe "with valid parameters and user permission" do
      ...
      describe "for code requests" do
        ...
        it "redirects to the client with an authorization code" do
          response = allow_or_deny(params)
          expect(response.code.to_i).to eql(302)
          expect(response['location']).to eql('https://client.example.com/cb?code=foo')
        end
        ...
        it "passes the state parameter through" do
          ...
          response = allow_or_deny(params)
          expect(response.code.to_i).to eql(302)
          expect(response['location']).to eql('https://client.example.com/cb?code=foo&state=illinois')
        end
        ...
        it "passes the scope parameter through" do
          ...
          response = allow_or_deny(params)
          expect(response.code.to_i).to eql(302)
          expect(response['location']).to eql('https://client.example.com/cb?code=foo&scope=foo+bar')
        end
      end
      ...
      describe "for token requests" do
        ...
        it "redirects to the client with an access token" do
          response = allow_or_deny(params)
          expect(response.code.to_i).to eql(302)
          expect(response['location']).to eql('https://client.example.com/cb#access_token=foo&expires_in=10800')
        end
        ...
        it "passes the state parameter through" do
          ...
          response = allow_or_deny(params)
          expect(response.code.to_i).to eql(302)
          expect(response['location']).to eql('https://client.example.com/cb#access_token=foo&expires_in=10800&state=illinois')
        end
        ...
        it "passes the scope parameter through" do
          ...
          response = allow_or_deny(params)
          expect(response.code.to_i).to eql(302)
          expect(response['location']).to eql('https://client.example.com/cb#access_token=foo&expires_in=10800&scope=foo+bar')
        end
      end
      ...
      describe "for code_and_token requests" do
        ...
        it "redirects to the client with an access token" do
          response = allow_or_deny(params)
          expect(response.code.to_i).to eql(302)
          expect(response['location']).to eql('https://client.example.com/cb?code=foo#access_token=foo&expires_in=10800')
        end
        ...
        it "passes the state parameter through" do
          ...
          response = allow_or_deny(params)
          expect(response.code.to_i).to eql(302)
          expect(response['location']).to eql('https://client.example.com/cb?code=foo&state=illinois#access_token=foo&expires_in=10800')
        end
        ...
        it "passes the scope parameter through" do
          ...
          response = allow_or_deny(params)
          expect(response.code.to_i).to eql(302)
          expect(response['location']).to eql('https://client.example.com/cb?code=foo#access_token=foo&expires_in=10800&scope=foo+bar')
        end
      end
    end
  end
    ...
    describe "using authorization_code request" do
      ...
      describe "with valid parameters" do
        it "does not respond to GET" do
          expect(Songkick::OAuth2::Provider::Authorization).not_to receive(:new)
      ...
      describe "when there is an Authorization with code and token" do
        ...
        it "exchanges the code for the new token on the existing Authorization" do
          ...
          expect(@authorization.access_token_hash).to eql(Songkick::OAuth2.hashify('random_access_token'))
        end
      end
    end
  end
  ...
  describe "protected resource request" do
    ...
    shared_examples_for "protected resource" do
      ...
      it "allows access when the key is passed" do
        ...
        expect(JSON.parse(response.body)['data']).to eql('Top secret')
        expect(response.code.to_i).to eql(200)
      end

      it "blocks access when the wrong key is passed" do
        response = request('/user_profile', 'oauth_token' => 'is-the-password-books')
        expect(JSON.parse(response.body)['data']).to eql('No soup for you')
        expect(response.code.to_i).to eql(401)
        expect(response['WWW-Authenticate']).to eql("OAuth realm='Demo App', error='invalid_token'")
      end
      ...
      it "blocks access when the no key is passed" do
        ...
        expect(JSON.parse(response.body)['data']).to eql('No soup for you')
        expect(response.code.to_i).to eql(401)
        expect(response['WWW-Authenticate']).to eql("OAuth realm='Demo App'")
      end
      ...
      describe "enforcing SSL" do
        ...
        it "blocks access when not using HTTPS" do
          ...
          expect(JSON.parse(response.body)['data']).to eql('No soup for you')
          expect(response.code.to_i).to eql(401)
          expect(response['WWW-Authenticate']).to eql("OAuth realm='Demo App', error='invalid_request'")
        end

        it "destroys the access token since it's been leaked" do
          expect(authorization.access_token_hash).to eql(Songkick::OAuth2.hashify('magic-key'))
          request('/user_profile', 'oauth_token' => 'magic-key')
          authorization.reload
