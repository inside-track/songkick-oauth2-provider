...
describe Songkick::OAuth2::Provider::Authorization do
  ...
  describe "with the scope parameter" do
    ...
    it "exposes the scope as a list of strings" do
      expect(authorization.scopes).to eql(Set.new(%w[foo bar qux]))
    end

    it "exposes the scopes the client has not yet granted" do
      expect(authorization.unauthorized_scopes).to eql(Set.new(%w[foo bar qux]))
    end
    ...
    describe "when the owner has already authorized the client" do
      ...
      it "exposes the scopes the client has not yet granted" do
        expect(authorization.unauthorized_scopes).to eql(%w[qux])
      end
    end
  end
  ...
  describe "missing response_type" do
    ...
    it "is invalid" do
      expect(authorization.error).to eql("invalid_request")
      expect(authorization.error_description).to eql("Missing required parameter response_type")
    end
  end
  ...
  describe "with a bad response_type" do
    ...
    it "is invalid" do
      expect(authorization.error).to eql("unsupported_response_type")
      expect(authorization.error_description).to eql("Response type no_such_type is not supported")
    end

    it "causes a redirect" do
      expect(authorization).to be_redirect
      expect(authorization.redirect_uri).to eql("https://client.example.com/cb?error=unsupported_response_type&error_description=Response+type+no_such_type+is+not+supported")
    end
  end
  ...
  describe "missing client_id" do
    ...
    it "is invalid" do
      expect(authorization.error).to eql("invalid_request")
      expect(authorization.error_description).to eql("Missing required parameter client_id")
    end
  ...
  describe "with an unknown client_id" do
    ...
    it "is invalid" do
      expect(authorization.error).to eql("invalid_client")
      expect(authorization.error_description).to eql("Unknown client ID unknown")
    end
  ...
  describe "missing redirect_uri" do
    ...
    it "is invalid" do
      expect(authorization.error).to eql("invalid_request")
      expect(authorization.error_description).to eql("Missing required parameter redirect_uri")
    end

    it "causes a redirect to the client's registered redirect_uri" do
      expect(authorization).to be_redirect
      expect(authorization.redirect_uri).to eql("https://client.example.com/cb?error=invalid_request&error_description=Missing+required+parameter+redirect_uri")
    end
  end
  ...
  describe "with a mismatched redirect_uri" do
    ...
    it "is invalid" do
      expect(authorization.error).to eql("redirect_uri_mismatch")
      expect(authorization.error_description).to eql("Parameter redirect_uri does not match registered URI")
    end

    it "causes a redirect to the client's registered redirect_uri" do
      expect(authorization).to be_redirect
      expect(authorization.redirect_uri).to eql("https://client.example.com/cb?error=redirect_uri_mismatch&error_description=Parameter+redirect_uri+does+not+match+registered+URI")
    end
  ...
  describe "with a redirect_uri with parameters" do
    ...
    it "adds the extra parameters with & instead of ?" do
      expect(authorization.redirect_uri).to eql("http://songkick.com?some_parameter&")
    end
  end
  ...
  describe "with an illegal scope" do
    ...
    it "is invalid" do
      expect(authorization.error).to eql("invalid_request")
      expect(authorization.error_description).to eql("Illegal value for scope parameter")
    end
  end
  ...
  describe "with an illegal state" do
    ...
    it "is invalid" do
      expect(authorization.error).to eql("invalid_request")
      expect(authorization.error_description).to eql("Illegal value for state parameter")
    end
  end
  ...
  describe "#grant_access!" do
    describe "when there is an existing authorization with no code" do
      ...
      it "generates and returns a code to the client" do
        ...
        @model.reload
        expect(@model.code).to eql("s1")
        expect(authorization.code).to eql("s1")
      end
    end
    ...
    describe "when there is an existing authorization with scopes" do
      ...
      it "merges the new scopes with the existing ones" do
        authorization.grant_access!
        @model.reload
        expect(@model.scopes).to eql(Set.new(%w[foo bar qux]))
      end
    end
    ...
    describe "when there is an existing expired authorization" do
      ...
      it "returns a code to the client" do
        authorization.grant_access!
        expect(authorization.code).to eql("existing_code")
      ...
      it "sets the expiry time if a duration is given" do
        ...
        @model.reload
        expect(@model.expires_in).to eql(3600)
        expect(authorization.expires_in).to eql(3600)
      end
    ...
    describe "for code requests" do
      ...
      it "creates an Authorization in the database" do
        ...
        expect(authorization.owner).to eql(resource_owner)
        expect(authorization.client).to eql(@client)
        expect(authorization.code).to eql("s1")
        expect(authorization.scopes).to eql(Set.new(%w[foo bar]))
      end
    end
    ...
    describe "for token requests" do
      ...
      it "creates a token for the authorization" do
        ...
        expect(authorization.access_token).to eql("s1")
        expect(authorization.refresh_token).to eql("s2")
        expect(authorization.expires_in).to be_nil
      end
      ...
      it "creates an Authorization in the database" do
        ...
        expect(authorization.client).to eql(@client)
        expect(authorization.code).to be_nil
        expect(authorization.access_token_hash).to eql(Songkick::OAuth2.hashify("s1"))
        expect(authorization.refresh_token_hash).to eql(Songkick::OAuth2.hashify("s2"))
      end
    end
    ...
    describe "for code_and_token requests" do
      ...
      it "creates a code and token for the authorization" do
        authorization.grant_access!
        expect(authorization.code).to eql("s1")
        expect(authorization.access_token).to eql("s2")
        expect(authorization.refresh_token).to eql("s3")
        expect(authorization.expires_in).to be_nil
      end
      ...
      it "creates an Authorization in the database" do
        ...
        expect(authorization.client).to eql(@client)
        expect(authorization.code).to eql("s1")
        expect(authorization.access_token_hash).to eql(Songkick::OAuth2.hashify("s2"))
        expect(authorization.refresh_token_hash).to eql(Songkick::OAuth2.hashify("s3"))
      end
    end
  end
  ...
  describe "#deny_access!" do
    it "puts the authorization in an error state" do
      ...
      expect(authorization.error).to eql("access_denied")
      expect(authorization.error_description).to eql("The user denied you access")
    end

    it "does not create an Authorization" do
      expect(Songkick::OAuth2::Model::Authorization).not_to receive(:create)
      expect(Songkick::OAuth2::Model::Authorization).not_to receive(:new)
      authorization.deny_access!
    end
  end
  ...
  describe "#params" do
    ...
    it "only exposes OAuth-related parameters" do
      expect(authorization.params).to eql({
        'response_type' => 'code',
        'client_id'     => @client.client_id,
        'redirect_uri'  => @client.redirect_uri,
        'state'         => 'valid',
