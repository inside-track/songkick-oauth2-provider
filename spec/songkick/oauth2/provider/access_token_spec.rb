...
describe Songkick::OAuth2::Provider::AccessToken do
  ...
  shared_examples_for "valid token" do
    ...
    it "does not add headers" do
      expect(token.response_headers).to eql({})
    end
    it "has an OK status code" do
      expect(token.response_status).to eql(200)
    end
    it "returns the owner who granted the authorization" do
      expect(token.owner).to eql(@bob)
    end
  end
  ...
  describe "with no user" do
    ...
    it "returns an error response" do
      expect(token.response_headers['WWW-Authenticate']).to eql("OAuth realm='Demo App', error='invalid_token'")
      expect(token.response_status).to eql(401)
    end
  end
  ...
  describe "when the authorization has expired" do
    ...
    it "returns an error response" do
      expect(token.response_headers['WWW-Authenticate']).to eql("OAuth realm='Demo App', error='expired_token'")
      expect(token.response_status).to eql(401)
    end
  end
  ...
  describe "with a token for the wrong user" do
    ...
    it "returns an error response" do
      expect(token.response_headers['WWW-Authenticate']).to eql("OAuth realm='Demo App', error='insufficient_scope'")
      expect(token.response_status).to eql(403)
    end
  end
  ...
  describe "with no token string" do
    ...
    it "returns an error response" do
      expect(token.response_headers['WWW-Authenticate']).to eql("OAuth realm='Demo App'")
      expect(token.response_status).to eql(401)
    end
  end
end
